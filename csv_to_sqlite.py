...
def get_latest_name(evals, subject, subject_number):
    ...
    matches = evals.query(
        f"subject=='{subject}' and subject_number=='{subject_number}'"
    )
...
def create_evals_table(evals):
    ...
    evals["avg_weekly_workload"] = (
        (0.5 * (evals["zero_one"] / 100))
...
def create_class_evals_table(evals):
    ...
    for major in sorted(majors):
        ...
        for row in major_list.itertuples():
            overall_avg_percentile = scipy.stats.percentileofscore(
                a=major_list.overall_avg, score=row.overall_avg
            )
            difficulty_avg_percentile = scipy.stats.percentileofscore(
                a=major_list.difficulty_avg, score=row.difficulty_avg
            )
            avg_weekly_workload_percentile = scipy.stats.percentileofscore(
                a=major_list.avg_weekly_workload, score=row.avg_weekly_workload
            )

            class_evals["overall_avg_percentile"].loc[
                row.Index
            ] = overall_avg_percentile
            class_evals["difficulty_avg_percentile"].loc[
                row.Index
            ] = difficulty_avg_percentile
            class_evals["avg_weekly_workload_percentile"].loc[
                row.Index
            ] = avg_weekly_workload_percentile

    for row in class_evals.itertuples():
        class_eval = ClassEval(
            subject=row.Index[0],
...
def create_professor_evals_table(evals):
    professor_evals = pd.pivot_table(
        evals,
    ...
    for major in sorted(majors):
        ...
        for row in major_list.itertuples():
            ...
            professor_evals["overall_avg_percentile"].loc[
                row.Index
            ] = overall_avg_percentile
            professor_evals["difficulty_avg_percentile"].loc[
                row.Index
            ] = difficulty_avg_percentile
            professor_evals["avg_weekly_workload_percentile"].loc[
                row.Index
            ] = avg_weekly_workload_percentile

    for id, row in enumerate(professor_evals.itertuples()):
        professor_eval = ProfessorEval(
            id=id,
...
def create_course_professor_table(evals):
    course_professor_evals = pd.pivot_table(
        evals,
    ...
    for major in sorted(majors):
        ...
        for row in major_list.itertuples():
            ...
            course_professor_evals["overall_avg_percentile"].loc[
                row.Index
            ] = overall_avg_percentile
            course_professor_evals["difficulty_avg_percentile"].loc[
                row.Index
            ] = difficulty_avg_percentile
            course_professor_evals["avg_weekly_workload_percentile"].loc[
                row.Index
            ] = avg_weekly_workload_percentile
